...
export default function AdminMarketplace() {
  ...
  const [config, setConfig] = useState({ commissionRate: 12 });
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [merchantsData, statsData, configData] = await Promise.all([
        businessRules.getAdminMerchants(),
        businessRules.getAdminMarketplaceStats(),
  ...
  const handleUpdateStatus = async (merchantId: string, currentStatus: string) => {
    const newStatus = currentStatus === 'active' ? 'inactive' : 'active';
    try {
      await businessRules.updateMerchantStatus(merchantId, newStatus);
  ...
  const handleApprove = async (merchantId: string) => {
    try {
      await businessRules.updateMerchantStatus(merchantId, 'active');
      toast.success('Lojista aprovado com sucesso!');
      loadData();
    } catch (error) {
      toast.error('Erro ao aprovar lojista.');
    }
  };
  ...
  const handleUpdateCommission = async () => {
    ...
    if (isNaN(rate)) return toast.error('Valor inválido.');
  ...
  const statsDisplay = [
    { label: 'GMV Mensal', value: `R$ ${stats?.gmv?.value?.toLocaleString('pt-BR') || '0'}`, trend: `${(stats?.gmv?.trend || 0) >= 0 ? '+' : ''}${stats?.gmv?.trend?.toFixed(1) || '0'}%`, isPositive: (stats?.gmv?.trend || 0) >= 0, icon: TrendingUp, color: 'text-indigo-500' },
    { label: 'Lojistas Ativos', value: stats?.merchants?.value?.toString() || '0', trend: 'Estável', isPositive: true, icon: Store, color: 'text-emerald-500' },
    { label: 'Produtos Ativos', value: stats?.products?.value?.toString() || '0', trend: 'Estável', isPositive: true, icon: Package, color: 'text-purple-500' },
    { label: 'Ticket Médio', value: `R$ ${stats?.avgTicket?.value?.toFixed(2).replace('.', ',') || '0,00'}`, trend: 'Baseado em vendas', isPositive: true, icon: ShoppingBag, color: 'text-amber-500' },
  ];
          ...
          <div className="flex flex-col lg:flex-row items-center justify-between gap-8 relative z-10">
            ...
            <div className="flex flex-wrap justify-center gap-4">
              <button 
                ...
                className="px-8 py-4 bg-white/5 hover:bg-white/10 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest border border-white/5 transition-all flex items-center gap-2 group"
              >
                <FolderTree size={16} className="text-indigo-400 group-hover:scale-110 transition-transform" />
                Categorias
              </button>
              <button 
                onClick={handleUpdateCommission}
                disabled={isSaving}
                className="px-8 py-4 bg-indigo-600 hover:bg-indigo-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all shadow-xl shadow-indigo-600/20 disabled:opacity-50"
              >
                {isSaving ? 'Salvando...' : 'Ajustar Taxas'}
              </button>
            </div>
          </div>
        </div>
                ...
                {loading ? (
                  <tr>
                    <td colSpan={7} className="py-20 text-center">
                      <div className="flex flex-col items-center gap-4">
                        ...
                        <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Carregando lojistas...</p>
                      </div>
                    </td>
                  </tr>
                  ...
                  filteredMerchants.map((merchant) => (
                    <tr key={merchant.id} className="group hover:bg-white/[0.02] transition-colors">
                      <td className="py-6 px-8">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest">ID: {merchant.id.substring(0, 8).toUpperCase()}</p>
                          </div>
                        </div>
                      </td>
                      <td className="py-6 px-8">
                      ...
                      <td className="py-6 px-8 font-black text-white text-sm">R$ {merchant.sales.toLocaleString('pt-BR')}</td>
                      <td className="py-6 px-8">
                        ...
                        <div className="flex items-center justify-end gap-2">
                          {merchant.status === 'under_review' ? (
                            <button 
                              ...
                              className="p-2 bg-emerald-500/10 text-emerald-500 hover:bg-emerald-500 hover:text-white rounded-xl transition-all" title="Aprovar Lojista"
                            >
                              <CheckCircle size={18} />
                            </button>
                          ) : (
                            <button 
                              onClick={() => handleUpdateStatus(merchant.id, merchant.status)}
                              className={`p-2 bg-white/5 rounded-xl transition-all ${merchant.status === 'active' ? 'text-red-400 hover:text-red-500' : 'text-emerald-400 hover:text-emerald-500'}`}
                              title={merchant.status === 'active' ? 'Suspender' : 'Reativar'}
                            >
                              {merchant.status === 'active' ? <AlertTriangle size={18} /> : <CheckCircle size={18} />}
                            </button>
                          )}
                  ...
                  <tr>
                    <td colSpan={7} className="py-20 text-center text-slate-500 text-[10px] font-black uppercase tracking-widest">Nenhum lojista encontrado</td>
                  </tr>
                )}
              </tbody>
            </table>
