            ...
            <motion.div initial={{opacity:0, scale:0.9, y: 20}} animate={{opacity:1, scale:1, y: 0}} exit={{opacity:0, scale:0.9, y: 20}} className="relative bg-white w-full max-w-2xl rounded-[2.5rem] overflow-hidden shadow-2xl flex flex-col max-h-[90vh]">
              ...
              <div className="p-8 bg-slate-50 border-t border-slate-100 flex gap-4">
                ...
                {selectedOrder.status !== 'Concluído' && (
                  <button 
                    onClick={() => {
                      handleUpdateStatus(selectedOrder.id, 'Concluído');
                      setShowDetailsModal(false);
                    }}
                    className="flex-1 px-8 py-4 bg-emerald-500 text-white rounded-2xl text-[10px] font-black uppercase tracking-widest shadow-xl shadow-emerald-500/20 hover:scale-105 transition-all"
                  >
