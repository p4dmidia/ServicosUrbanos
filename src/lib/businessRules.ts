...
export const businessRules = {
  ...
  async getAllOrders() {
    ...
    return (data || []).map(o => ({
      ...
      order_date: o.order_date || o.date || o.created_at,
  ...
  getAdminReportsData: async (range: string, customStartDate?: string, customEndDate?: string) => {
    ...
    const getOrderDateStr = (o: any) => (o.order_date || o.created_at || o.date || '').substring(0, 10);
    const startDateStr = customStartDate ? customStartDate : startDate.toISOString().substring(0, 10);
    const endDateStr = customEndDate ? customEndDate : endDate.toISOString().substring(0, 10);
  ...
  getOrdersBreakdownForAffiliate: async (userId: string, targetRefMonth?: string) => {
    try {
      const isDecember = targetRefMonth ? targetRefMonth.endsWith('-12') : false;
      const targetYear = targetRefMonth ? parseInt(targetRefMonth.split('-')[0], 10) : new Date().getFullYear();

      // 1. Tentar obter via transactions registradas para o usuário
      ...
      let historyComms = (userTxs || []).filter((h: any) => h.type === 'commission' && Number(h.amount) > 0);

      if (targetRefMonth && historyComms.length > 0) {
        const [yStr, mStr] = targetRefMonth.split('-');
        const monthPrefix = `/${mStr.padStart(2, '0')}/${yStr}`;
        const yyyyMm = `${yStr}-${mStr.padStart(2, '0')}`;
        historyComms = historyComms.filter((h: any) => {
          const d = h.created_at ? new Date(h.created_at).toLocaleDateString('pt-BR') : '';
          const raw = h.created_at || '';
          const isSameMonth = raw.startsWith(yyyyMm) || d.includes(monthPrefix);
          const isAnual = (h.description || '').toLowerCase().includes('anual');

          if (isDecember) {
            // Em Dezembro: traz transações mensais de Dezembro + TODAS as transações anuais do ano corrente
            const txYear = new Date(raw || Date.now()).getFullYear();
            return isSameMonth || (isAnual && txYear === targetYear);
          } else {
            // Meses regulares (Jan a Nov): somente transações do próprio mês e sem anual
            return isSameMonth && !isAnual;
          }
        });
      }
      ...
      // 2. Se transactions estiver vazio, busca diretamente na tabela orders da rede e polo do afiliado
      const [{ data: allOrders }, { data: allProfiles }] = await Promise.all([
      ...
      (allOrders || []).forEach((o: any) => {
        ...
        if (targetRefMonth) {
          const [y, m] = targetRefMonth.split('-');
          const monthPrefix = `/${m.padStart(2, '0')}/${y}`;
          const yyyyMm = `${y}-${m.padStart(2, '0')}`;
        ...
        if (isOwn) {
          // Comissão mensal (5%) só entra se for do próprio mês
          if (isSameMonth) {
            breakdown.push({
              id: `ord-${o.id}-g0`,
              orderId: String(o.id),
          ...
          if (isReseller) {
            // Venda direta mensal (10%) só entra se for do próprio mês
            if (isSameMonth) {
              breakdown.push({
      ...
      console.error('Erro ao buscar breakdown de pedidos:', err);
      return [];
    }
  },
  ...
  getAffiliateRPAReceipts: async (userId?: string, referenceMonth?: string): Promise<RPAReceipt[]> => {
    ...
    try {
      ...
      console.warn('Erro ao consultar RPAs no Supabase:', err);
    }

    // 2. Mescla com localStorage
    try {
      const localRpas: RPAReceipt[] = JSON.parse(localStorage.getItem('all_rpa_receipts') || '[]');
      localRpas.forEach(lr => {
        const key = `${lr.profile_id}_${lr.reference_month}`;
        const existing = rpaMap.get(key);
        if (!existing) {
          rpaMap.set(key, lr);
        } else if (lr.status === 'quitado' && existing.status !== 'quitado') {
          rpaMap.set(key, lr);
        }
      });
    } catch (e) {}

    allRpas = Array.from(rpaMap.values());

    // Atualiza o localStorage local com os dados consolidados do banco
    try {
  ...
  generateMonthlyRPAReceipt: async (userId: string, refMonth?: string): Promise<RPAReceipt | null> => {
    try {
      ...
      // 3. Obter detalhamento completo e resiliente de pedidos vinculados ao RPA
      const ordersBreakdown = await (businessRules as any).getOrdersBreakdownForAffiliate(userId, targetRefMonth);
      const ordersList = ordersBreakdown.map((o: any) => o.orderNumber);

      // 4. Apura os rendimentos do mês selecionado
    ...
    try {
      ...
      try {
        await supabase
          ...
          .delete()
          .eq('profile_id', userId)
          .eq('reference_month', target.reference_month);

        const payload = {
          profile_id: userId,
          reference_month: target.reference_month,
          amount_gross: target.financial.liquido_total || target.financial.bruto_total || 0,
  ...
  getConsolidatedFinancialStatement: async (userId: string, year: number, month: number) => {
    ...
    const allPeriodComms = [
      ...monthMmnCommissions.map(t => ({ ...t, categoryType: 'MMN_MENSAL' })),
      ...monthResellerCommissions.map(t => ({ ...t, categoryType: 'RESELLER_MENSAL' })),
      ...annualMmnComms.map(t => ({ ...t, categoryType: 'MMN_ANUAL' })),
      ...annualResellerComms.map(t => ({ ...t, categoryType: 'RESELLER_ANUAL' }))
    ];

    // Busca dados complementares dos pedidos
    const orderIdCandidates: string[] = [];
    allPeriodComms.forEach(t => {
      if (t.order_id) orderIdCandidates.push(String(t.order_id));
      const desc = t.description || '';
      const match = desc.match(/#([a-zA-Z0-9-]+)/);
      if (match) orderIdCandidates.push(match[1]);
    });

    const uniqueOrderIds = Array.from(new Set(orderIdCandidates.filter(Boolean)));
    ...
    if (uniqueOrderIds.length > 0) {
      try {
        const { data: ordersData } = await supabase
          .from('orders')
          .select('id, amount, customer_name, customer_id, created_at, status')
          .in('id', uniqueOrderIds);
        (ordersData || []).forEach(o => ordersMap.set(String(o.id), o));
      } catch (e) {
        console.error('Erro ao buscar pedidos para o demonstrativo consolidado:', e);
      }
    }

    const ordersBreakdown = allPeriodComms.map(t => {
      let rawOrderId = t.order_id ? String(t.order_id) : '';
      if (!rawOrderId) {
        const match = (t.description || '').match(/#([a-zA-Z0-9-]+)/);
        if (match) rawOrderId = match[1];
      }

      const order = rawOrderId ? ordersMap.get(rawOrderId) : null;
      let orderNumber = rawOrderId 
        ? (rawOrderId.length > 12 ? `#${rawOrderId.substring(0, 8)}` : (rawOrderId.startsWith('#') ? rawOrderId : `#${rawOrderId}`))
        : `Ped. #${t.id ? t.id.substring(0, 6) : '---'}`;

      const desc = t.description || '';
      let origin = 'Rede MMN';
      if (t.categoryType?.includes('RESELLER') || desc.toLowerCase().includes('revendedor') || desc.toLowerCase().includes('regional')) {
        origin = 'Revenda Regional';
      } else if (desc.includes('G0') || desc.includes('Nível 0') || desc.includes('Próprio') || (order && order.customer_id === userId)) {
        origin = 'MMN G0 (Próprio)';
      } else if (desc.includes('G1') || desc.includes('Nível 1') || desc.includes('1º')) {
        origin = 'MMN G1 (1º Nível)';
      } else if (desc.includes('G2') || desc.includes('Nível 2') || desc.includes('2º')) {
        origin = 'MMN G2 (2º Nível)';
      } else if (t.metadata?.level !== undefined) {
        origin = `MMN G${t.metadata.level}`;
      }

      const orderAmount = Number(order?.amount || t.metadata?.order_amount || 0);
      const commissionAmount = Number(t.amount || 0);
      let commissionRate = 0;
      if (orderAmount > 0) {
        commissionRate = Number(((commissionAmount / orderAmount) * 100).toFixed(1));
      } else if (t.metadata?.rate) {
        commissionRate = Number(t.metadata.rate);
      } else {
      ...
      return {
        ...
        orderNumber: orderNumber,
        date: new Date(t.created_at).toLocaleDateString('pt-BR'),
        customerName: order?.customer_name || t.metadata?.customer_name || 'Assinante / Cliente',
        origin: origin,
        type: t.categoryType?.includes('ANUAL') ? 'Cashback Anual' : 'Cashback Mensal',
    ...
    return {
      ...
      ordersList,
      ordersBreakdown,
      ordersCount: ordersBreakdown.length,
      periodoStr,
      previsaoPagamentoStr,
      limiteNotaFiscalStr,
      isDecemberAnnualPayout,
