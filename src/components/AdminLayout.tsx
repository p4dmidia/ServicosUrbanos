...
export default function AdminLayout({ children, title, subtitle }: AdminLayoutProps) {
  ...
  const menuItems = [
    ...
    { name: 'Configuração MMN', icon: Zap, path: '/admin/mmn' },
        ...
        <div className="p-6 border-t border-white/5 space-y-4">
          <button 
            ...
            className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl text-xs font-black uppercase tracking-widest transition-all ${
              isActive('/admin/configuracoes') ? 'bg-white/10 text-white' : 'text-slate-500 hover:text-white hover:bg-white/5'
            }`}
          >
            <Settings size={18} />
            Configurações
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-[radial-gradient(circle_at_top_right,_var(--tw-gradient-stops))] from-indigo-500/5 via-transparent to-transparent">
        ...
        <header className="bg-[#0a0e17]/80 backdrop-blur-xl border-b border-white/5 py-6 px-6 lg:px-12 flex items-center justify-between sticky top-0 z-30">
          ...
          <div className="flex items-center gap-4 lg:gap-6">
            ...
            <button className="relative size-10 bg-white/5 border border-white/5 rounded-xl flex items-center justify-center text-white hover:bg-white/10 transition-colors">
              <Bell size={18} />
