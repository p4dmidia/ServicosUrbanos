...
async function check() {
  const { data, error } = await supabase.from('profiles').select('id, full_name, email, cpf, cnpj, description, store_name');
  if (error) {
  ...
  data.forEach(p => {
    console.log(`ID: ${p.id} | Name: "${p.full_name}" | CPF: "${p.cpf}" | CNPJ: "${p.cnpj}" | Desc: "${p.description}"`);
  });
}
